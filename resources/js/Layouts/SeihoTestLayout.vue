...
const openGoogleForm = () => {
  const googleFormUrl = 'https://docs.google.com/forms/d/e/1FAIpQLSdYvdwHrldz56rBizNEgfUx9o5e9WdME_OwXpdjyv7oWlW3mQ/viewform?usp=header';
...
<template>
    ...
    <div class="scroll-smooth min-h-screen bg-gradient-to-br from-gray-50 via-white to-purple-50">
        <!-- ナビゲーションバー - モダンデザイン -->
        <header class="sticky top-0 z-30 bg-white/80 backdrop-blur-lg border-b border-gray-100 shadow-sm">
            <div class="container mx-auto flex justify-between items-center px-6 md:px-8 lg:px-16 py-4">
                <Link :href="route('tests.index')" class="flex items-center gap-3 group">
                    <div class="relative">
                        <div class="absolute inset-0 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-xl blur opacity-30 group-hover:opacity-50 transition-opacity"></div>
                        <img
                    ...
                    <span class="text-xl font-black bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-purple-600">
                        生保講座過去問解説
                    </span>
                </Link>

                <button @click="toggleMenu" class="md:hidden p-2 rounded-xl hover:bg-gray-100 transition-colors">
        ...
        <transition name="fade">
            <div v-if="isMenuOpen" class="fixed inset-0 z-40 bg-black/50 backdrop-blur-sm" @click="toggleMenu">
                <aside class="fixed top-0 right-0 w-80 h-full bg-gradient-to-b from-gray-900 via-gray-900 to-indigo-950 shadow-2xl z-50 overflow-y-auto" @click.stop>

                    <!-- ヘッダー -->
                    <div class="relative bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-6 px-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <h2 class="text-2xl font-black">メニュー</h2>
                                <p class="text-sm text-indigo-100 mt-1">科目から選択</p>
                            </div>
                            <button @click="toggleMenu" class="p-2 hover:bg-white/20 rounded-xl transition-colors">
                    ...
                    <div class="px-4 py-6 space-y-2">
                        <div v-for="subject in subjects" :key="subject.key" class="rounded-2xl overflow-hidden">
                            <button
                                ...
                                :class="openSubjects.has(subject.key) ? 'bg-white/10' : ''"
                            >
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <svg
                                        ...
                                        :class="openSubjects.has(subject.key) ? 'rotate-180' : ''"
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                    >
                            ...
                            <transition name="slide-down">
                                <div v-if="openSubjects.has(subject.key)" class="px-3 pb-3 space-y-3">
                                    <div v-for="(forms, yearLabel) in subject.tests" :key="yearLabel"
                                         class="bg-white/5 rounded-xl p-3">
                                        <div class="text-white font-bold text-sm mb-2 px-2 flex items-center gap-2">
            ...
            <div class="container mx-auto px-6 py-12">
                <div class="flex flex-col md:flex-row md:justify-between md:items-center space-y-8 md:space-y-0">
                    ...
                    <div class="flex items-center justify-center md:justify-start space-x-3">
                        <div class="relative">
                            <div class="absolute inset-0 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-xl blur opacity-30"></div>
                            <img
                                src="/images/rencon3.png"
                                alt="れんこん"
                        ...
                        <div class="flex flex-col text-left">
                            <span class="text-base font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-purple-600">
                                生保講座過去問解説
                            </span>
                        ...
                        <div class="bg-gradient-to-r from-gray-50 to-purple-50 rounded-2xl p-6 border border-gray-100">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="text-xs text-gray-600 leading-relaxed space-y-1">
                                    <p>当サイトは受験者の学習支援を目的としたものであり、生命保険協会等の公式機関とは一切関係ありません。</p>
                                    <p>問題文等の転載は行っておらず、独自に作成した解説コンテンツを掲載しています。</p>
                                    <p>内容の正確性・完全性は保証されませんのでご了承ください。</p>
                                </div>
                            </div>
                        </div>
                    </div>
